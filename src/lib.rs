...
impl Renderer {
    pub fn new(
        ...
        let vs_module =
            device.create_shader_module(&Shaders::compile_glsl(vs_code, ShaderStage::Vertex));
        let fs_module =
            device.create_shader_module(&Shaders::compile_glsl(fs_code, ShaderStage::Fragment));
